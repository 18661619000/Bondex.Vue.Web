...
import { login, getUserByEmail, logout } from '@/api/login'
import { ACCESS_TOKEN, LOGIN_USER } from '@/store/mutation-types'
import { welcome, builder } from '@/utils/util'
...
const user = {
  ...
  actions: {
    ...
    GetInfo ({ commit }) {
      return new Promise((resolve, reject) => {
        getUserByEmail(storage.get(LOGIN_USER).email).then(response => {
        const result = response.Data
          ...
          commit('SET_NAME', { name: result.NameCN, welcome: welcome() })
          commit('SET_AVATAR', 'avatar2.jpg')
          commit('SET_OPERATORS', result.Operators)
